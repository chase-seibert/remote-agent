...
Conversation messages use the same bubble hierarchy as iOS: the user's blue, white-text bubbles align to the right, while agent and system bubbles use semantic gray backgrounds on the left. Headers include role and time. Responses use a native Markdown block renderer that preserves paragraph spacing, headings, unordered and ordered lists, block quotes, dividers, fenced code blocks, inline emphasis, links, and inline code. Plain-text line breaks remain visible, and the original response remains selectable and available through the Copy context-menu action. While a turn runs, a rotating green sparkle and “Agent is working…” appear together in a green pill.
