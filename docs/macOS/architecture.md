...
- `SessionStore` atomically persists Remote Agent's session metadata, visible transcripts, and queued prompts under Application Support. Codex separately persists its own conversation/tool state.
...
- `RemoteAPIServer` is a small HTTP/1.1 server built on Network.framework. It listens on IPv4/IPv6 with peer-to-peer support, advertises `_remoteagent._tcp` through Bonjour, accepts one request per connection, and delegates state changes to `AppModel`. Authenticated prompt-queue collection and item routes create, list, edit, and delete pending work. Parsing caps headers at 32 KB and bodies at 2 MB, rejects invalid framing, and times out incomplete requests after 15 seconds.
...
6. Clients observe the running turn and remaining host-owned queue through their next session poll. `SessionStore` never writes `currentReasoning` to disk.
