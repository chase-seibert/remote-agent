...
Sessions have contextual actions for pinning or unpinning, creating another session in the same project, revealing the project in Finder, marking results read, and deleting the session. Deletion requires destructive confirmation and is unavailable while the session is running. A toolbar item and Command-N open a native project picker before creating a session, Command-R refreshes project discovery, and Command-Return sends a prompt. Conversation text supports selection and copy. Command-Plus, Command-Minus, and Command-0 adjust a persisted interface text scale, also available in Settings. The scale applies to session titles, projects, activity timestamps, status badges, transcript labels and content, progress status, and the prompt composer.

The conversation toolbar includes a native Make split button plus separate Commit and Push buttons, mirrored in the Project menu. The Make button label contains the active target; its primary region runs that target immediately, while its arrow menu changes the target remembered for that session. Commit stages the full working tree with `git add --all` and generates a short subject with Apple's on-device Foundation Models framework. Project actions are disabled while the agent or another project command is running.
