...
3. `Packages/RemoteAgentProtocol` owns API details that must remain identical on both sides, including the protocol version, endpoint construction, queued-prompt wire model, and mutation request contracts.

The Mac host remains the authority for projects, sessions, transcripts, unread state, pin state, prompt queues, Make target selection, project-command execution, and captured output. The iOS app caches server snapshots and command configuration while persisting only mobile-local composer drafts. Each app retains its own build definition and release workflow; the root Makefile coordinates compatible builds and tests without merging their runtime responsibilities.
