...
- `SessionStore` atomically persists Remote Agent's session metadata, visible transcripts, queued prompts, and a monotonic content revision under Application Support. Every API-visible session mutation advances that revision; transient reasoning is returned directly by the status endpoint and does not advance it. Codex separately persists its own conversation/tool state.
