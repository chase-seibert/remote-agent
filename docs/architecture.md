...
The Mac host remains the authority for projects, sessions, transcripts, unread state, pin state, Make target selection, project-command execution, and captured output. The iOS app caches server snapshots and command configuration while persisting only mobile-local drafts and queued prompts. Each app retains its own build definition and release workflow; the root Makefile coordinates compatible builds and tests without merging their runtime responsibilities.
